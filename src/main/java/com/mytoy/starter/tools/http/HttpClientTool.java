package com.mytoy.starter.tools.http;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.TypeReference;
import com.google.common.collect.Lists;
import com.mytoy.starter.tools.MyJSON;
...
public class HttpClientTool {
     ...
     * 文件上傳
     */
    public HttpClientTool doPostFormMultipart(List<File> files) {
        url = perfectUrl(url);
        HttpPost httpPost = new HttpPost(url);
        ...
        if (MapUtils.isNotEmpty(params))
            params.keySet().forEach(key -> mEntityBuilder.addTextBody(key, params.get(key), contentType));
        if (CollectionUtils.isNotEmpty(files)) files.forEach(file -> mEntityBuilder.addBinaryBody("file", file));
    ...
    public void getContent(HttpRequestBase httpBase, String encode) {
        ...
        log.info("http请求地址:{},请求头{},请求参数{}", url, MyJSON.toJSONString(headers), params.size() > 0 ? MyJSON.toJSONString(params) : "" + (bodyJson == null ? "" : bodyJson));
